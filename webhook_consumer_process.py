...
def main():
    ...
    result = channel.queue_declare(queue='webhook')
            ...
            try:
                with httpx.Client() as client:
                    ...
                    if response.is_success:
                        print(f" [x] Webhook notification sent successfully: {body}")
                    else:
                        print(f" [x] Failed to send Webhook notification. Status code: {response.status_code}")
            except Exception as e:
                print(f" [x] Failed to send Webhook notification. Error: {str(e)}")
...
if __name__ == '__main__':
    try:
        ...
        print('\nWebhook consumer process exited')
        try:
            sys.exit(0)
        except SystemExit:
            os._exit(0)